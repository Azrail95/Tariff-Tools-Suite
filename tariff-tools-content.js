...
(function() {
    ...
    class TariffExporter {
        ...
        getStandardButtonClasses() {
            const disableBtn = this.findDisableAllButton();
            if (disableBtn) {
                return {
                    classes: disableBtn.className,
                    hasDiv: disableBtn.querySelector('div') !== null
                };
            }
            return { classes: '_defaultButton_o6g7k_1 _small_o6g7k_49 _withText_o6g7k_43', hasDiv: true };
        }
        ...
        addNavigationButtons() {
            if (document.querySelector('#tariff-export-pro-nav-btn') && 
                ...
                document.querySelector('#tariff-rename-pro-nav-btn')) return;
            ...
            const actionRow = standardRow.parentNode;   // В организацию + стандартные кнопки
            ...
            // Группа из двух рядов: 1) существующий actionRow, 2) наш extraRow.
            // Стандартные кнопки внутри actionRow не трогаем.
            ...
            this.importButton.id = 'tariff-create-pro-nav-btn';
            this.importButton.className = standardClasses.classes;

            const importInnerDiv = document.createElement('div');
            importInnerDiv.style.cssText = 'display: flex; gap: 4px; flex-direction: row;';
            const importSpan = document.createElement('span');
            importSpan.textContent = '➕ Массовое создание';
            importInnerDiv.appendChild(importSpan);
            this.importButton.appendChild(importInnerDiv);

            this.importButton.style.background = 'linear-gradient(135deg, #10b981 0%, #059669 100%)';
            this.importButton.style.border = 'none';
            this.importButton.onmouseenter = () => {
                this.importButton.style.background = 'linear-gradient(135deg, #34d399 0%, #10b981 100%)';
            };
            this.importButton.onmouseleave = () => {
                this.importButton.style.background = 'linear-gradient(135deg, #10b981 0%, #059669 100%)';
            };
            this.importButton.onclick = () => {
                this.showImportSidebar();
            };
            this.importButton.style.marginRight = '0px';
            targetContainer.appendChild(this.importButton);

            // Переименовать
            this.renameButton = document.createElement('button');
            this.renameButton.id = 'tariff-rename-pro-nav-btn';
            this.renameButton.className = standardClasses.classes;

            const renameInnerDiv = document.createElement('div');
            renameInnerDiv.style.cssText = 'display: flex; gap: 4px; flex-direction: row;';
            const renameSpan = document.createElement('span');
            renameSpan.textContent = '✏️ Переименовать';
            renameInnerDiv.appendChild(renameSpan);
            this.renameButton.appendChild(renameInnerDiv);

            this.renameButton.style.background = 'linear-gradient(135deg, #8b5cf6 0%, #7c3aed 100%)';
            this.renameButton.style.border = 'none';
            this.renameButton.onmouseenter = () => {
                this.renameButton.style.background = 'linear-gradient(135deg, #a78bfa 0%, #8b5cf6 100%)';
            };
            this.renameButton.onmouseleave = () => {
                this.renameButton.style.background = 'linear-gradient(135deg, #8b5cf6 0%, #7c3aed 100%)';
            };
            this.renameButton.onclick = () => {
                this.showRenameSidebar();
            };
            this.renameButton.style.marginRight = '0px';
            targetContainer.appendChild(this.renameButton);

            // Импорт
            this.updateButton = document.createElement('button');
            this.updateButton.id = 'tariff-import-pro-nav-btn';
            this.updateButton.className = standardClasses.classes;

            const updateInnerDiv = document.createElement('div');
            updateInnerDiv.style.cssText = 'display: flex; gap: 4px; flex-direction: row;';
            const updateSpan = document.createElement('span');
            updateSpan.textContent = '🔄 Импорт';
            updateInnerDiv.appendChild(updateSpan);
            this.updateButton.appendChild(updateInnerDiv);

            this.updateButton.style.background = 'linear-gradient(135deg, #f59e0b 0%, #d97706 100%)';
            this.updateButton.style.border = 'none';
            this.updateButton.onmouseenter = () => {
                this.updateButton.style.background = 'linear-gradient(135deg, #fbbf24 0%, #f59e0b 100%)';
            };
            this.updateButton.onmouseleave = () => {
                this.updateButton.style.background = 'linear-gradient(135deg, #f59e0b 0%, #d97706 100%)';
            };
            this.updateButton.onclick = () => {
                this.showUpdateSidebar();
            };
            this.updateButton.style.marginRight = '0px';
            targetContainer.appendChild(this.updateButton);

            // Экспорт
            this.navButton = document.createElement('button');
            this.navButton.id = 'tariff-export-pro-nav-btn';
            this.navButton.className = standardClasses.classes;

            const exportInnerDiv = document.createElement('div');
            exportInnerDiv.style.cssText = 'display: flex; gap: 4px; flex-direction: row;';
            const exportSpan = document.createElement('span');
            exportSpan.textContent = this.isExporting ? '📊 Экспорт (активен)' : '📊 Экспорт';
            exportInnerDiv.appendChild(exportSpan);
            this.navButton.appendChild(exportInnerDiv);

            this.navButton.style.background = this.isExporting ? 
                'linear-gradient(135deg, #10b981 0%, #059669 100%)' : 
                'linear-gradient(135deg, #4b6e8a 0%, #2c3e50 100%)';
            this.navButton.style.border = 'none';

            this.navButton.onmouseenter = () => {
                if (!this.isExporting) {
                    this.navButton.style.background = 'linear-gradient(135deg, #5d7f9e 0%, #3a546d 100%)';
                }
            };
            this.navButton.onmouseleave = () => {
                if (!this.isExporting) {
                    this.navButton.style.background = 'linear-gradient(135deg, #4b6e8a 0%, #2c3e50 100%)';
                }
            };
            ...
            this.navButton.style.marginRight = '0px';
            targetContainer.appendChild(this.navButton);

            console.log('[TariffExportPro] Кнопки собраны в 2 ряда');
        }
...
// Кнопка массового переименования
this.renameButton = document.createElement('button');
this.renameButton.id = 'tariff-rename-pro-nav-btn';
this.renameButton.className = standardClasses.classes;
...
renameInnerDiv.style.cssText = 'display: flex; gap: 4px; flex-direction: row;';
const renameSpan = document.createElement('span');
renameSpan.textContent = '✏️ Переименование';
renameInnerDiv.appendChild(renameSpan);
this.renameButton.appendChild(renameInnerDiv);
...
// Кнопка реального импорта (обновление)
this.updateButton = document.createElement('button');
this.updateButton.id = 'tariff-import-pro-nav-btn';
this.updateButton.className = standardClasses.classes;
...
div.insertBefore(this.updateButton, div.firstChild);
        ...
        updateButtonsForExportState() {
            ...
            console.log('[TariffExportPro] Обновление кнопок:', {
                isExporting: this.isExporting,
                tariffsDataLength: this.tariffsData.length,
                totalTariffs: this.totalTariffs,
                showSave: this.tariffsData.length > 0 && !this.isExporting
            });
            ...
            if (this.isExporting) {
                ...
                // Есть собранные данные и экспорт не активен - показываем кнопку сохранения
                if (startBtn) startBtn.style.display = 'none';
                if (stopBtn) stopBtn.style.display = 'none';
                if (saveBtn) saveBtn.style.display = 'block';
            } else {
                // Нет данных, экспорт не активен - показываем кнопку старта
        ...
        handleCollectedData(dataStr) {
            ...
            try {
                ...
                if (data.tariffData && data.requestId) {
                    // Не логируем успех здесь: основной цикл already adds
                    // `✅ Данные собраны ...` после await openAndCollect(...).
                    // Иначе в сайдбаре появляются дубли для каждого тарифа.
        ...
        async startExport() {
            ...
            try {
                ...
                // Важно: сначала обновляем отображение, потом кнопки
                this.updateSidebarDisplay();
                ...
                if (error.message === 'Экспорт остановлен' || error.message === 'Экспорт остановлен пользователем') {
                    ...
                    // Если есть частично собранные данные, показываем кнопку сохранения
                ...
                // Важно: обновляем состояние кнопок после завершения
                this.updateSidebarDisplay();
                this.updateButtonsForExportState();
                
                // Дополнительная проверка: если есть данные, но кнопка сохранения не появилась
                setTimeout(() => {
                    if (this.tariffsData.length > 0 && !this.isExporting) {
                        console.log('[TariffExportPro] Принудительное обновление кнопок');
                        this.updateButtonsForExportState();
                        if (this.sidebar) {
                            const saveBtn = document.getElementById('sidebar-save');
                            if (saveBtn && saveBtn.style.display !== 'block') {
        ...
        saveExcelFile() {
            ...
            // После успешного сохранения сразу очищаем состояние,
            // чтобы следующий экспорт всегда начинался с чистого листа.
